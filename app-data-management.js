...
const AUTO_BACKUP_PREF_KEY="wf_auto_backup_enabled",AUTO_BACKUP_HASH_KEY="wf_auto_backup_data_hash",AUTO_BACKUP_CHECK_KEY="wf_auto_backup_last_check",AUTO_BACKUP_PROMPT_KEY="wf_auto_backup_last_prompt",AUTO_BACKUP_INTERVAL_MS=4*60*60*1000;
...
function renderAutomaticBackupPermission(){if(localStorage.getItem(AUTO_BACKUP_PREF_KEY)!==null)return;let host=document.getElementById("backupReminder")||document.getElementById("automaticBackupControls");if(!host||document.getElementById("automaticBackupPermission"))return;let box=document.createElement("div");box.id="automaticBackupPermission";box.className="notice backup-reminder-banner";box.innerHTML=`🔄 هل تسمح للنظام بفحص تغييرات البيانات كل 4 ساعات وطلب موافقتك قبل تنزيل نسخة احتياطية؟<div class="backup-reminder-actions"><button type="button" class="primary small-btn" id="enableAutomaticBackup">تفعيل النسخ التلقائي</button><button type="button" class="secondary small-btn" id="skipAutomaticBackup">لا الآن</button></div>`;host.appendChild(box);box.querySelector("#enableAutomaticBackup").addEventListener("click",()=>{localStorage.setItem(AUTO_BACKUP_PREF_KEY,"yes");box.remove();renderAutomaticBackupControls();runAutomaticBackupCheck(true)});box.querySelector("#skipAutomaticBackup").addEventListener("click",()=>{localStorage.setItem(AUTO_BACKUP_PREF_KEY,"no");box.remove();renderAutomaticBackupControls()})}
...
function initAutomaticBackup(){renderAutomaticBackupControls();if(localStorage.getItem(AUTO_BACKUP_PREF_KEY)===null){renderAutomaticBackupPermission();return}if(!automaticBackupEnabled())return;runAutomaticBackupCheck();scheduleAutomaticBackupCheck()}
