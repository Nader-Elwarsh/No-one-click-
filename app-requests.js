...
// المصنعية والعربون كانوا أرقام ثابتة في صفحة الأمر، وأي تعديل عليهم كان
// لازم يدخل وضع التعديل الكامل من فوق. هنا بنعيد حساب الإجمالي والمتبقي
// تلقائيًا زي ما بالظبط بيحصل وقت إضافة/حذف قطعة غيار من نفس الصفحة —
// من غير أي مزامنة مع المحافظ (نفس سياسة تعديل القطع الحالية بالظبط).
function changeRequestLabor(i,val){
  let a=arr(K.r),r=a.find(x=>x.id===i);if(!r||r.closed||r.paid)return;
  let labor=+val;if(!Number.isFinite(labor)||labor<0)labor=0;
  let total=labor+(+r.partsTotal||0);
  r.labor=labor;r.total=total;r.remain=Math.max(0,total-(+r.deposit||0));
  if(!saveJSONSafe(K.r,a))return;requestProfile();renderRequests();
}
function changeRequestDeposit(i,val){
  let a=arr(K.r),r=a.find(x=>x.id===i);if(!r||r.closed||r.paid)return;
  let deposit=+val;if(!Number.isFinite(deposit)||deposit<0)deposit=0;
  r.deposit=deposit;r.remain=Math.max(0,(+r.total||0)-deposit);
  if(!saveJSONSafe(K.r,a))return;requestProfile();renderRequests();
}
...
function requestCommActionsHtml(r,custPhone){
  ...
  return `<div class="request-comm-actions no-print">${waRow}<div class="receipt-actions-row"><button type="button" class="secondary mini-action" data-wf-event="click" data-wf-code="printReceiptView('${r.id}')">🖨️ طباعة الإيصال</button><button type="button" class="secondary mini-action" data-wf-event="click" data-wf-code="shareReceiptView('${r.id}')">↗️ مشاركة الإيصال</button></div></div>`;
}
